...
function vehicles.upgrade(veh)
    ...
    local p = math.random(1, #vehicle_colors)
    VEHICLE.SET_VEHICLE_WINDOW_TINT(veh, 6)
    VEHICLE.SET_VEHICLE_TYRES_CAN_BURST(veh, false)
    VEHICLE.SET_VEHICLE_EXTRA_COLOURS(veh, enums.vehicle_colors["Secret Gold"], enums.vehicle_colors["Secret Gold"])
    VEHICLE.SET_VEHICLE_COLOURS(veh, enums.vehicle_colors["Secret Gold"], enums.vehicle_colors["Secret Gold"])
    ...
    VEHICLE._SET_VEHICLE_NEON_LIGHTS_COLOUR(veh, vehicle_colors[p][1], vehicle_colors[p][2], vehicle_colors[p][3])
end
...
function vehicles.get_first_free_seat(vehicle)
    ...
    if VEHICLE.ARE_ANY_VEHICLE_SEATS_FREE(vehicle) == 0 then return end
    for i = -1, max do
...
function vehicles.get_player_vehicle(player)
	...
	if PED.IS_PED_IN_ANY_VEHICLE(PLAYER.GET_PLAYER_PED(player), true) == 0 then return 0 end
...
for _, v in ipairs(vehicles.models) do
    table.insert(v, enums.vehicle_class[VEHICLE.GET_VEHICLE_CLASS_FROM_NAME(v[2])])
end
