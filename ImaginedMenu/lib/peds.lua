...
ped.GetPedName = switch()
    ...
    :case("ig_nervousron_02", "cs_nervousron_02", function()
        return "Nervous Ron 2"
    end)
    ...
    :case("ig_drfriedlander_02", "cs_drfriedlander_02", function()
        return "Dr. Friedlander 2"    
    end)
    ...
    :case("u_f_m_debbie_01", function()
        return "Debbie (Agatha's Secretary)"
    end)
    ...
    :case("ig_chef_03", "csb_chef_03", function()
        return "Chef 3"
    end)
    ...
    :case("a_f_m_genstreet_01", function()
        return "Street Female"
    end)
    :case("a_c_chimp_02", function()
        return "Chimp 2"
    end)
    :case("a_c_rabbit_02", function()
        return "Rabbit 2"
    end)
    :case("a_c_rabbit_02", function()
        return "Rabbit 2"
    end)
    :case("csb_dax", "ig_dax", function()
        return "Dax"
    end)
    :case("csb_labrat", "ig_labrat", function()
        return "Laboratory Rat"
    end)
    :case("csb_luchadora", "ig_luchadora", function()
        return "Lauchadora"
    end)
    :case("g_f_m_fooliganz_01", "ig_fooliganz_01", "ig_fooliganz_02", function()
        return "Female Fooliganz"
    end)
    :case("g_m_m_fooliganz_01", function()
        return "Male Fooliganz"
    end)
    :case("g_m_m_friedlandergoons_01", function()
        return "Friedlander Goons"
    end)
    :case("s_m_m_hazmatworker_01", function()
        return "Hazmat Worker"
    end)
    :case("ig_acidlabcook", function()
        return "Acid Lab Cook"
    end)
    :case("ig_furry", function()
        return "Furry"
    end)
    :case("ig_gunvanseller", function()
        return "Gun Van Seller"
    end)
    :case("ig_hippyleader", function()
        return "Hippy Leader"
    end)
    :case("u_m_m_yulemonster", function()
        return "Gooch"
    end)
    :case("a_m_m_bankrobber_01", function()
        return "Bank Robber"
    end)
...
ped.movement_clip_sets = {
    ...
    { "Film Female", "move_f@film_reel" },
    { "Cool Jog", "move_m@jog@" },
    { "Leaf Blower", "move_m@leaf_blower" },
    { "Tool Belt Walk", "move_m@tool_belt@a" },
    { "Tool Belt Walk 2", "move_f@tool_belt@a" },
    { "Appealing", "move_f@sexy@a" },
    { "Amanda - Bag", "move_characters@amanda@bag" },
    { "Michael - Fire", "move_characters@michael@fire" },
    { "Franklin - Fire", "move_characters@franklin@fire" },
    { "Jimmy - Nervous", "move_characters@jimmy@nervous@" },
    { "Jimmy - Slow", "move_characters@jimmy@slow@" },
    { "Alien", "move_m@alien" },
    { "Brave", "move_m@brave" },
    { "Brave2", "move_m@brave@a" },
    { "Brave3", "move_m@brave@b" },
    { "Brave4", "move_m@brave@fallback" },
    { "BraveStill", "move_m@brave@idle_a" },
    { "BraveStill2", "move_m@brave@idle_b" },
    ...
    { "Hiking2", "move_f@hiking" },
    { "Hurry", "move_m@hurry@a" },
    { "Hurry2", "move_m@hurry@b" },
    { "Hurry3", "move_m@hurry@c" },
    { "Hurry4", "move_f@hurry@a" },
    { "Hurry5", "move_f@hurry@b" },
    { "Injured", "move_m@injured" },
    { "Injured2", "move_injured_generic" },
    { "Injured3", "move_f@injured" },
    { "Intimidation", "move_m@intimidation@1h" },
    { "Intimidation2", "move_m@intimidation@cop@unarmed" },
    { "Intimidation3", "move_m@intimidation@unarmed" },
...
--- @param tabl table
--- @return table containing peds names  
...
function ped.is_ped_an_enemy(cped)
    local relationship = PED.GET_RELATIONSHIP_BETWEEN_PEDS(features.player_ped(), cped)
    return ENTITY.IS_ENTITY_DEAD(cped, false) == 0 and ((relationship == 4 or relationship == 5) or PED.IS_PED_IN_COMBAT(cped, features.player_ped()) == 1)
end
...
--- @param cped Ped
--- @param toggle bool
function ped.set_can_ragdoll(cped, toggle)
    PED.SET_PED_CAN_RAGDOLL(cped, toggle)
    PED.SET_PED_CAN_RAGDOLL_FROM_PLAYER_IMPACT(cped, toggle)
end
...
function ped.play_scenario(cped, scenario)
    TASK.TASK_START_SCENARIO_IN_PLACE(cped, scenario, -1, false)
end

--- @param cped Ped
--- @param anim_dict string
--- @param anim_name string
--- @param blend_in_speed float
--- @param blend_out_speed float
--- @param duration float
--- @param flags int
--- @param playback_rate float
--- @param lock bool
function ped.play_anim(cped, anim_dict, anim_name, blend_in_speed, blend_out_speed, duration, flags, playback_rate, lock)
    if TASK.IS_PED_ACTIVE_IN_SCENARIO(cped) == 1 then
        TASK.PLAY_ANIM_ON_RUNNING_SCENARIO(cped, anim_dict, anim_name)
    else
        TASK.TASK_PLAY_ANIM(cped, anim_dict, anim_name, blend_in_speed, blend_out_speed, duration, flags, playback_rate, lock, lock, lock)
    end
end

--- @param cped Ped
function ped.revive(cped)
    local anim_dict = "get_up@directional@movement@from_knees@standard"
    local anim_name = "getup_r_90"
    local tick = 0
    system.thread(true, 'revive_'..cped, function()
        ...
        tick = tick + 1
        if tick == 1 then
            entities.request_control(cped, function()
                ENTITY.SET_ENTITY_HEALTH(cped, 200, 0)
                ENTITY.SET_ENTITY_MAX_HEALTH(cped, 400)
                PED.RESURRECT_PED(cped)
                PED.REVIVE_INJURED_PED(cped)
                ENTITY.SET_ENTITY_HEALTH(cped, 200, 0)
                PED.SET_PED_GENERATES_DEAD_BODY_EVENTS(cped, false)
                PED.SET_PED_CONFIG_FLAG(cped, 166, false)
                PED.SET_PED_CONFIG_FLAG(cped, 187, false)
                TASK.CLEAR_PED_TASKS_IMMEDIATELY(cped)
                ped.play_anim(cped, anim_dict, anim_name, 8, -8, -1, enum.anim_flag.StopOnLastFrame, 0, false)
            end)
        end
        if tick < 100 then return end
        entities.request_control(cped, function()
            TASK.TASK_WANDER_STANDARD(cped, 10, 10)
        end)
...
--- @param cped? Ped
--- @return table
function ped.get_head_blend(cped)
    local head_blend_data = s_memory.alloc(10 * 8)
    PED.GET_PED_HEAD_BLEND_DATA(cped or features.player_ped(), head_blend_data)
    return {
        shapeFirst = memory.read_int(head_blend_data),
        shapeSecond = memory.read_int(head_blend_data + 8),
        shapeThird = memory.read_int(head_blend_data + 16),
        skinFirst = memory.read_int(head_blend_data + 24),
        skinSecond = memory.read_int(head_blend_data + 32),
        skinThird = memory.read_int(head_blend_data + 40),
        shapeMix = memory.read_float(head_blend_data + 48),
        skinMix = memory.read_float(head_blend_data + 56),
        thirdMix = memory.read_float(head_blend_data + 64),
        isParent = features.to_bool(memory.read_byte(head_blend_data + 72))
    }
end

--- @param cped Ped
--- @param data table
function ped.set_ped_head_blend(cped, data)
    PED.SET_PED_HEAD_BLEND_DATA(cped, data.shapeFirst, data.shapeSecond, data.shapeThird, data.skinFirst, data.skinSecond, data.skinThird, data.shapeMix, data.skinMix, data.thirdMix, data.isParent)
end

--- @param cped Ped
--- @return table
function ped.get_ped_overlay(cped)
    local overlay_data = {}
    for i = 0, 12
    do
        insert(overlay_data, PED._GET_PED_HEAD_OVERLAY_VALUE(cped, i))
    end
    return overlay_data
end
...
--- @param cped Ped
--- @param overlay table
function ped.apply_overlay(cped, overlay)
    for i, v in ipairs(overlay)
    do
        i = i - 1
        PED.SET_PED_HEAD_OVERLAY(cped, i, v, 255)
        if i == 1 or i == 2 or i == 10 then
            PED._SET_PED_HEAD_OVERLAY_COLOR(cped, i, 1, 0, 0)
        elseif i == 5 or i == 8 then
            PED._SET_PED_HEAD_OVERLAY_COLOR(cped, i, 2, 0, 0)
        else
            PED._SET_PED_HEAD_OVERLAY_COLOR(cped, i, 0, 0, 0)
        end
    end
end
...
--- @param cped? Ped
--- @return table
function ped.get_weapons(cped)
    local cped = cped or features.player_ped()
    local weapon = {}
    for _, v in ipairs(weapons.data)
    do
        if WEAPON.HAS_PED_GOT_WEAPON(cped, v.Hash, false) == 1 then
            local Components = {}
            for _, e in ipairs(v.Components)
            do
                if WEAPON.HAS_PED_GOT_WEAPON_COMPONENT(cped, v.Hash, e.Hash) == 1 then
                    insert(Components, e.Hash)
                end
            end
            insert(weapon, {
                Hash = v.Hash,
                Tint = WEAPON.GET_PED_WEAPON_TINT_INDEX(cped, v.Hash),
...
--- @param weapon_hash int
--- @param cped? Ped 
function ped.set_max_ammo(weapon_hash, cped)
    local cped = cped or features.player_ped()
    local ammo = WEAPON.GET_PED_AMMO_TYPE_FROM_WEAPON(cped, weapon_hash)
    WEAPON._ADD_AMMO_TO_PED_BY_TYPE(cped, ammo, 9999)
end

--- @param weapon table
--- @param cped? Ped 
function ped.set_weapons(weapon, cped)
    if not weapon then return end
    local cped = cped or features.player_ped()
    WEAPON.REMOVE_ALL_PED_WEAPONS(cped, false)
    for _, v in ipairs(weapon) do
        if WEAPON.HAS_PED_GOT_WEAPON(cped, v.Hash, false) == 0 then
            WEAPON.GIVE_DELAYED_WEAPON_TO_PED(cped, v.Hash, 9999, false)
            if v.Components then
                for _, e in ipairs(v.Components)
                do
                    if WEAPON.HAS_PED_GOT_WEAPON_COMPONENT(cped, v.Hash, e) == 0 then
                        WEAPON.GIVE_WEAPON_COMPONENT_TO_PED(cped, v.Hash, e)
                    end
                end
                WEAPON.SET_PED_WEAPON_TINT_INDEX(cped, v.Hash, v.Tint)
            end
        end
        ped.set_max_ammo(v.Hash, cped)
    end
end

--- @param cped? Ped
--- @return table
function ped.get_outfit(cped)
    local cped = cped or features.player_ped()
    local outfit = {components = {}, props = {}}
    for i = 0, 11 do
        outfit.components[tostring(i)] = {
            PED.GET_PED_DRAWABLE_VARIATION(cped, i),
            PED.GET_PED_TEXTURE_VARIATION(cped, i)
        }
    end
    for i = 0, 9 do
        outfit.props[tostring(i)] = {
            PED.GET_PED_PROP_INDEX(cped, i),
            PED.GET_PED_PROP_TEXTURE_INDEX(cped, i)
        }
    end
    return outfit
end
...
--- @param props table
--- @param cped? Ped
function ped.apply_outfit(components, props, cped)
    local cped = cped or features.player_ped()
    for k, v in pairs(components)
    do
        local i = tonumber(k)
        local var = PED.GET_PED_DRAWABLE_VARIATION(cped, i)
        local txt = PED.GET_PED_TEXTURE_VARIATION(cped, i)
        if var ~= v[1] or txt ~= v[2] then
            PED.SET_PED_COMPONENT_VARIATION(cped, i, v[1], v[2], 0)
        end
    end
    for k, v in pairs(props)
    do
        local i = tonumber(k)
        local prop = PED.GET_PED_PROP_INDEX(cped, i)
        local txt = PED.GET_PED_PROP_TEXTURE_INDEX(cped, i)
        if v[1] == -1 and prop ~= -1 then
            PED.CLEAR_PED_PROP(cped, i)
        elseif prop ~= v[1] or txt ~= v[2] then
            PED.SET_PED_PROP_INDEX(cped, i, v[1], v[2], true)
        end
    end
end
