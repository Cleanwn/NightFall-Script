...
function notify(text)
    ...
    system.notify("Additions Menu", str_text, 0, 255, 0, 255)
    system.log("Additions Menu", str_text)
end
...
notify("Additions Menu 成功加载！翻译者：MG")
...
main_sub = ui.add_main_submenu("Additions Menu")
...
ui.add_click_option("脚本冻结玩家", ply_mod, function()
	...
	local ind = PLAYER.GET_PLAYER_INDEX()
	local name = online.get_name(target)
	notify("Sending Script Freeze to " .. name .. "")
...
ui.add_click_option("在他面前放一堵墙", ply_trolling, function()
	local target = online.get_selected_player()
	local selfped = PLAYER.PLAYER_PED_ID()
	local selectedPlayer = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(target)
...
ui.add_click_option("Create Up-n-Atomizer Shot", ply_trolling, function()
	local target = online.get_selected_player()
	local selectedPlayer = PLAYER.GET_PLAYER_PED_SCRIPT_INDEX(target)
